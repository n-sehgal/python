...
def ListPossibleMoves(Board, PlayersPieces, NextPlayer, ListOfMoves):
    ...
    for i in range(1, NUMBER_OF_PIECES + 1):
        ...
        if Piece[0] == "A" or Piece[0] == "B":
            ForwardLeapRow = CurrentRow + 2*Direction
            ReverseLeapRow = CurrentRow - 2*Direction
            if ValidMove(Board, ForwardLeapRow, CurrentColumn) == True:
                print(Piece, ' can leap to ', ForwardLeapRow, ' , ', CurrentColumn)
                NumberOfMoves += 1
                ListOfMoves[NumberOfMoves].Piece = Piece
                ListOfMoves[NumberOfMoves].NewRow = ForwardLeapRow
                ListOfMoves[NumberOfMoves].NewColumn = CurrentColumn
                ListOfMoves[NumberOfMoves].CanJump = False
            if ValidMove(Board, ReverseLeapRow, CurrentColumn) == True:
                print(Piece, ' can leap to ', ReverseLeapRow, ' , ', CurrentColumn)
                NumberOfMoves += 1
                ListOfMoves[NumberOfMoves].Piece = Piece
                ListOfMoves[NumberOfMoves].NewRow = ReverseLeapRow
                ListOfMoves[NumberOfMoves].NewColumn = CurrentColumn
                ListOfMoves[NumberOfMoves].CanJump = False
